...
abstract class AbstractFractalView extends View {
	...
	public final int ZOOM_SLIDER_SCALING = 500;
	...
	// Dragging state
	public float currentDragX = 0;
	public float currentDragY = 0;
...
/*-----------------------------------------------------------------------------------*/
	...
	protected void onDraw(Canvas canvas) {	   
		...
		if(controlmode == ControlMode.STATIC && !holdingPin) {
			bitmapCreations++;
			String type = "null";
			if (thisFractal == FractalType.MANDELBROT){
				type = "Mandelbrot";
			}
			if (thisFractal == FractalType.JULIA){
				type = "Julia";
			}
			//Log.d("View TEST", "Type : " + type + " width = " + getWidth() + " height = " + getHeight());
	...
	public void computeAllPixels(final int pixelBlockSize, final int threadID) {
        ...
        if(fractalType == FractalType.MANDELBROT){
            ...
            magnification = this.parentActivity.jMagnification;
			Log.d("MagnificationTest"," Julia magnification = " + magnification);
...
/*-----------------------------------------------------------------------------------*/
	...
	public void dragFractal(float dragDiffPixelsX, float dragDiffPixelsY, float currentScreenCenterX, float currentScreenCenterY) {
		bitmapX = dragDiffPixelsX;
		bitmapY = dragDiffPixelsY;

		currentDragX = currentScreenCenterX;
		currentDragY = currentScreenCenterY;
	...
	public void shiftPixels(int shiftX, int shiftY) {
        ...
        Log.d("TEST1", "width = " + width + " shiftX = "+ shiftX + " colNum = " + colNum);
...
/*-----------------------------------------------------------------------------------*/
	...
	public void zoomChangePoint(double zoomCentreX, double zoomCentreY, float scale){
		stopAllRendering();
...
/*-----------------------------------------------------------------------------------*/
	...
	int getMaxIterations() {
		// How many iterations to perform?
		double absLnPixelSize = Math.abs(Math.log(getPixelSize())*2);
...
/*-----------------------------------------------------------------------------------*/
	...
	void setGraphArea(double[] newGraphArea, boolean newRender) {
		...
		if (graphArea != null && (fractalViewSize == FractalViewSize.LARGE || fractalViewSize == FractalViewSize.HALF)) {
...
/*-----------------------------------------------------------------------------------*/
	...
	protected void clearPixelSizes() {
	    Log.d("clearPixles Test", " pixleSizeArray Length = " + pixelSizes.length );
...
/*-----------------------------------------------------------------------------------*/
	...
	void computePixels(
		...
		int loopCount = 0;
		Log.d("TEST2", "computePixels: xPixle max : " + xPixelMax + " min : " + xPixelMin + " yPixle max : " + yPixelMax + " min : " + yPixelMin);
        double xScreenCenter = ((xPixelMax)/2);
        double yScreenCenter = ((yPixelMax)/2);
		...
		for (yIncrement = yPixelMin; yPixel < yPixelMax+(noOfThreads*pixelBlockSize) ; yIncrement += pixelIncrement) {			
			...
			for (xPixel=xPixelMin; xPixel<xPixelMax+1-pixelBlockSize; xPixel+=pixelBlockSize) {					
				...
				colourCodeHex = pixelInSet(xRotated, yRotated, maxIterations);
